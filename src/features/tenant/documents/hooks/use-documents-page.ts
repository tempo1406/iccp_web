...
export type ResourceDialogState =
  ...
  | {
      ...
      categoryDraft: { id: string; name?: string; description?: string | null };
    }
...
export function useDocumentsPage() {
  ...
  const [sidebarMode, setSidebarMode] = useState<'folder' | 'category'>('folder');
  ...
  const canCreateCategories = hasPermission(PERMISSIONS.DOCUMENTS_CATEGORIES_CREATE);
  const canEditCategories = hasPermission(PERMISSIONS.DOCUMENTS_CATEGORIES_UPDATE);
  const canDeleteCategories = hasPermission(PERMISSIONS.DOCUMENTS_CATEGORIES_DELETE);

  const treeQuery = useDocumentTree(undefined, { enabled: canListDocuments });
  const categoriesQuery = useCategories({ enabled: canListDocuments });
  const categoryDocumentsQuery = useDocumentList(
    selectedCategoryId ? { categoryId: selectedCategoryId } : undefined,
    { enabled: canListDocuments && sidebarMode === 'category' && !recoverySelected },
  );
  ...
  const currentExplorerItems = useMemo<ExplorerItem[]>(() => {
    ...
    if (sidebarMode === 'category') {
      const documents = recoverySelected
        ? recoveryDocuments
        : (categoryDocumentsQuery.data ?? []);
      return [...documents]
  ...
  const currentPanelLabel = useMemo(() => {
    if (recoverySelected) {
      if (sidebarMode === 'folder' && isSelectedFolderInRecovery && selectedFolderNode) {
        return `${selectedFolderNode.name} recovery contents`;
      }
      return 'Recovery';
    }
    if (sidebarMode === 'category') {
      if (!selectedCategoryId) return 'All documents';
      return categories.find((c) => c.id === selectedCategoryId)?.name ?? 'Category';
    }
    if (!selectedFolderId) return 'Root library';
  ...
  function handleDeleteCategory(categoryId: string) {
    setConfirmDialogState({
      ...
      description:
        'Documents remain available, but this category label will be detached from them.',
  ...
  async function handleLibraryDrop(event: React.DragEvent<HTMLElement>) {
    ...
    openUploadDialog({
      folderId: sidebarMode === 'folder' ? selectedFolderId : undefined,
      categoryId:
        sidebarMode === 'category' ? (selectedCategoryId ?? undefined) : undefined,
      initialFiles: files,
    });
  ...
  return {
    ...
    isLoading: treeQuery.isPending || categoriesQuery.isPending,
    // permissions
    canListDocuments,
    canUploadDocuments,
    canEditDocuments,
